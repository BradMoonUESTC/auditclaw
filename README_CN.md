...
`auditclaw` 是一个面向代码审计场景的 agent-driven 核心扫描框架。它最核心的价值是：每个审计师都可以把自己的审计知识、检查思路和提示词方法写进 auditor 里的 `Markdown` 文件，做出一个真正属于自己的 AI 扫描引擎，而不需要自己重写整套 agent 运行框架。
...
这意味着它不是一个只有少量可调参数的通用扫描器，而是一套能让每个审计师把自己的风格、标准、经验和审计路径固化进去的 AI 扫描引擎框架。

当前仓库是有意聚焦在 `Audit Core` 上的，也就是核心扫描与审计执行层。它负责把审计方法论真正跑起来，同时对外提供清晰的接入方式，让 `openclaw` 这类系统在需要目标接入、任务编排、调度或平台工作流时，可以直接接到这个引擎上。
...
核心思想就是：审计师写 `md`，框架负责把这些知识编排成可重复执行、并且真正体现该审计师方法论的 AI 扫描引擎。
...
可以把现在的 `auditclaw` 理解为一个可嵌入、可扩展的核心引擎，而不是要去包揽全部平台职责的完整产品。它开放 HTTP、stdio 和 Python API，就是为了让 `openclaw` 这样的上层系统可以直接接入，而不是重复实现底层扫描引擎。
...
如果需要把核心能力暴露给外部系统，还可以启动 HTTP 服务或 stdio RPC 桥接层。这些接口存在的目的，就是让 `openclaw` 一类上层系统可以直接接入 `auditclaw`。
